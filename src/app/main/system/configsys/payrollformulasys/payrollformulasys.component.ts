...
@Component({
  selector: "sys-payrollformula",
  templateUrl: "./payrollformulasys.component.html",
  styleUrls: ["./payrollformulasys.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class PayrollFormulaSysComponent implements OnInit {
  // Varriable Language
  languages: any;
  ...
  viewRecord = (event: any) => {
    ...
    const objParamAdd = {areaId: this.modelAdd.areaId, id: this.modelAdd.id, type: "view" };
    const paramAdd = window.btoa(JSON.stringify(objParamAdd));
    this.router.navigate(["/sys/configsys/payrollformulasys/", paramAdd]);
  };

  // Build Toolbar
  buildToolbar = () => {
    const toolbarList = [ToolbarItem.EDIT, ToolbarItem.LOCK];
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(state, "hr/salarytypesys/GetAll");
  };

  public dataStateChange(state: DataStateChangeEventArgs): void {
    this.pageIndex = Math.floor(state.skip! / state.take!);
    let extraParams: any[] = [];
    this._coreService.execute(state, "hr/salarytypesys/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/sys/configsys/payrollformulasys/new"]);
        break;
      case ToolbarItem.EDIT:
        const selectRows = this.gridInstance.getSelectedRecords();
        if (selectRows && selectRows.length > 0) {
          this.modelAdd = selectRows[0];
          const objParamAdd = {areaId: this.modelAdd.areaId, id: this.modelAdd.id, type: "edit" };
          const paramAdd = window.btoa(JSON.stringify(objParamAdd));
          this.router.navigate([
            "/sys/configsys/payrollformulasys/",
            paramAdd,
          ]);
        } else {
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/salarytype/ChangeStatus", ids)
            .subscribe((res: any) => {
