...
@Component({
  selector: "sys-salaryelement",
  templateUrl: "./salaryelementsys.component.html",
  styleUrls: ["./salaryelementsys.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class SalaryElementSysComponent implements OnInit {
  // Varriable Language
  languages: any;
  ...
  public lstArea: any[] = [];
  public search: {     areaId: null | number } = {
    areaId: null
  }
  // query auto complete
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/sys/configsys/salaryelementsys/", paramAdd]);
  };

  // Build Toolbar
  buildToolbar = () => {
    const toolbarList = [ToolbarItem.ADD, ToolbarItem.EDIT, ToolbarItem.LOCK];
    this.toolbar = this.globals.buildToolbar("salary_element", toolbarList!);
  };
  // filter checkbox
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(state, "hr/salaryelementsys/GetAll",extraParams);
  };
  ...
  public GetlstArea = ()=>{
    this._coreService.Get("package/otherlist/GetAreas").subscribe((res: any) => {
                this.lstArea = res.data;  
                this.lstArea.unshift(
                  {
                    id: 0,
                    name: "Tất cả"
                  }
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/sys/configsys/salaryelementsys/new"]);
        break;
      case ToolbarItem.EDIT:
        ...
        if (selectRows && selectRows.length > 0) {
          ...
          this.router.navigate([
            "/sys/configsys/salaryelementsys/",
            paramAdd,
          ]);
        } else {
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/salaryelementsys/ChangeStatus", ids)
            .subscribe((res: any) => {
