...
import { SalaryElement } from "src/app/_models/app/system";
setCulture("en");

@Component({
  selector: "sys-salaryelement-edit",
  templateUrl: "./salaryelementsys-edit.component.html",
  styleUrls: ["./salaryelementsys-edit.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class SalaryElementSysEditComponent implements OnInit {
  // Varriable Language
  flagState = "";
  ...
  model: SalaryElement = new SalaryElement();
  modelTemp: SalaryElement = new SalaryElement();
  languages: any;
  ...
  loadData() {
    Promise.all([this.getById(), this.getGroupElement(),this.getListArea()]).then((res: any[]) => {
  ...
  getById() {
    return new Promise<void>((resolve) => {
      if (this.paramId) {
        this._coreService
          .Get("hr/salaryelementsys/get?id=" + this.paramId)
          .subscribe((res: any) => {
            resolve(res.data);
          });
      } else {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["sys/configsys/salaryelementsys"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState === "new") {
      this._coreService.Post("hr/salaryelementsys/add", param).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/salaryelementsys/Update", param).subscribe(
        (res: any) => {
