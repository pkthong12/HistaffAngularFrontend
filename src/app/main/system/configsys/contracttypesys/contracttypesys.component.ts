...
@Component({
  selector: "cms-app-contracttypesys",
  templateUrl: "./contracttypesys.component.html",
  styleUrls: ["./contracttypesys.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class ContractTypeSysComponent implements OnInit {
  // Varriable Language
  languages: any;
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/sys/configsys/contracttypesys/", paramAdd]);
  };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(state, "hr/contracttypesys/GetAll");
  };

  public dataStateChange(state: DataStateChangeEventArgs): void {
    this.pageIndex = Math.floor(state.skip! / state.take!);
    let extraParams: any[] = [];
    this._coreService.execute(state, "hr/contracttypesys/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/sys/configsys/contracttypesys/new"]);
        break;
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/contracttypesys/ChangeStatus", ids)
            .subscribe((res: any) => {
