...
@Component({
  selector: "cms-app-timetypesys",
  templateUrl: "./timetypesys.component.html",
  styleUrls: ["./timetypesys.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class TimeTypeSysComponent implements OnInit {
  // Varriable Language
  languages: any;
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/sys/configsys/timetype/", paramAdd]);
  };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(state, "hr/timetype/GetAll");
  };

  public dataStateChange(state: DataStateChangeEventArgs): void {
    this.pageIndex = Math.floor(state.skip! / state.take!);
    let extraParams: any[] = [];
    this._coreService.execute(state, "hr/timetype/GetAll", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/sys/configsys/timetype/new"]);
        break;
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("hr/timetype/ChangeStatus", ids)
            .subscribe((res: any) => {
