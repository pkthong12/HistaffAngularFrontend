...
@Component({
  selector: "cms-app-themeblog",
  templateUrl: "./themeblog.component.html",
  styleUrls: ["./themeblog.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class ThemeBlogComponent implements OnInit {
  // Varriable Language
  languages: any;
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/sys/website/themeblog/", paramAdd]);
  };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(state, "hr/themeblog/GetAll");
  };
  public dataStateChange(state: DataStateChangeEventArgs): void {
    this.pageIndex = Math.floor(state.skip! / state.take!);
    let extraParams: any[] = [];
    this._coreService.execute(state, "hr/themeblog/GetAll", extraParams);
  }
  // Event Click Toolbar
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/sys/website/themeblog/new"]);
        break;
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService.Post("hr/themeblog/ChangeStatus", ids).subscribe((res: any) => {
