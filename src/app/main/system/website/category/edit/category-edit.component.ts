...
import { Category } from "src/app/_models/app/system";
...
export class CategoryEditComponent implements OnInit {
  ...
  modelTemp: Category = new Category();
  languages: any;
  ...
  loadData() {
    Promise.all([
      new Promise((resolve) => {
        if (this.paramId) {
          this._coreService
            .PostNode("GetById", { _id: this.paramId })
            .subscribe((res: any) => {
              this.model = res.data;
            });
        }
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["sys/website/category/"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState === "new") {
      this._coreService
        .PostNode("createCategory", this.model)
        .subscribe((res: any) => {
      ...
      this._coreService
        .PostNode("editCategory", this.model)
        .subscribe((res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
