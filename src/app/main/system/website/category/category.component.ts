...
export class CategoryComponent implements OnInit {
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/sys/website/category/", paramAdd]);
  };
  ...
  buildToolbar = () => {
    ...
    this.toolbar = this.globals.buildToolbar("sys-app-Category", toolbarList!);
  };

  getListData = (): void => {
    this._coreService.GetNode("listCategory").subscribe((res: any) => {
      this.data = res.data;
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/sys/website/category/new"]);
        break;
      ...
      case ToolbarItem.DELETE:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService.PostNode("removeCategory", ids).subscribe((res: any) => {
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          let ids = selectDeletes[0];
          this._coreService.PostNode("activeCategory", ids).subscribe((res: any) => {
            if (res.statusCode == 400) {
              this.notification.checkErrorMessage(res.message);
            } else {
              this.notification.lockSuccess();
