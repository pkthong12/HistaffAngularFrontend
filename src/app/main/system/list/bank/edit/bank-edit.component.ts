...
import { Bank } from "src/app/_models/app/system/index";
...
export class BankEditComponent implements OnInit {
    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("hr/bank/get?id=" + this.paramId)
              .subscribe((res: any) => {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["sys/listsys/bank"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState === "new") {
      this._coreService.Post("hr/bank/add", param).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/bank/Update", param).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
