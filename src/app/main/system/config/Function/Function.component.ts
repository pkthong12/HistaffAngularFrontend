...
export class FunctionComponent implements OnInit {
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/sys/config/function/", paramAdd]);
  };
  ...
  buildToolbar = () => {
    ...
    this.toolbar = this.globals.buildToolbar("sys-app-Function", toolbarList!);
  };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(state, "author/function/getall");
  };

  public dataStateChange(state: DataStateChangeEventArgs): void {
    this.pageIndex = Math.floor(state.skip! / state.take!);
    let extraParams: any[] = [];
    this._coreService.execute(state, "author/function/getall", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/sys/config/function/new"]);
        break;
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("author/Function/changestatus", ids)
            .subscribe((res: any) => {
