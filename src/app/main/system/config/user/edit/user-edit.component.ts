...
export class UserEditComponent implements OnInit {
    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("authen/get?id=" + this.paramId)
              .subscribe((res: any) => {
  ...
  buildToolbar = () => {
    setTimeout(() => {
      ...
      this.toolbar = this.globals.buildToolbar("sys_user", toolbarList!);
    }, 200);
  };
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["sys/config/user"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState === "new") {
      this._coreService.Post("authen/CreateUser", this.model).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("authen/UpdateUser", this.model).subscribe(
        (res: any) => {
          if (res.statusCode == 200) {
