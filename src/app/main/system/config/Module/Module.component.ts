...
export class ModuleComponent implements OnInit {
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/sys/config/module/", paramAdd]);
  };
  ...
  buildToolbar = () => {
    ...
    this.toolbar = this.globals.buildToolbar("sys-app-Module", toolbarList!);
  };
  // filter checkbox
  showHideBttonFilter = () => {
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(state, "package/Module/getall");
  };

  public dataStateChange(state: DataStateChangeEventArgs): void {
    this.pageIndex = Math.floor(state.skip! / state.take!);
    let extraParams: any[] = [];
    this._coreService.execute(state, "package/Module/getall", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/sys/config/module/new"]);
        break;
      ...
      case ToolbarItem.LOCK:
        if (selectDeletes && selectDeletes.length > 0) {
          ...
          this._coreService
            .Post("package/Module/changestatus", ids)
            .subscribe((res: any) => {
