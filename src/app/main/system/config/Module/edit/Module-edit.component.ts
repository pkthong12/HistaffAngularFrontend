...
import { Module } from "src/app/_models/app/list/index";
...
export class ModuleEditComponent implements OnInit {
    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("package/Module/get?id=" + this.paramId)
              .subscribe((res: any) => {
  ...
  buildToolbar = () => {
    setTimeout(() => {
      ...
      this.toolbar = this.globals.buildToolbar("app-Module-edit", toolbarList!);
    }, 200);
  };
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["sys/config/module"]);
        }
        break;
  ...
  saveData = () => {
    if (!this.editForm.valid) {
      ...
      if (this.flagState === "new") {
        this._coreService.Post("package/Module/add", this.model).subscribe(
          (res: any) => {
        ...
        this._coreService.Post("package/Module/update", this.model).subscribe(
          (res: any) => {
            if (res.statusCode == 200) {
