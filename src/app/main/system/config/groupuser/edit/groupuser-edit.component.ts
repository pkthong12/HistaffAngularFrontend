...
import { GroupUser } from "src/app/_models/app/list/index";
...
export class GroupUserEditComponent implements OnInit {
  ...
  model: GroupUser = new GroupUser();
  modelTemp: GroupUser = new GroupUser();
  languages: any;
    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("author/groupuser/get?id=" + this.paramId)
              .subscribe((res: any) => {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["sys/config/groupuser"]);
        }
        break;
  ...
  saveData = () => {
    if (!this.editForm.valid) {
      ...
      if (this.flagState === "new") {
        this._coreService.Post("author/groupuser/add", this.model).subscribe(
          (res: any) => {
        ...
        this._coreService.Post("author/groupuser/update", this.model).subscribe(
          (res: any) => {
            if (res.statusCode == 200) {
