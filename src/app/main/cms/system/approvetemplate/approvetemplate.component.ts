...
@Component({
  selector: "cms-app-approvetemplate",
  templateUrl: "./approvetemplate.component.html",
  styleUrls: ["./approvetemplate.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class ApproveTemplateComponent implements OnInit {
  // Varriable Language
  languages: any;
  ...
  viewRecord = (event: any) => {
    ...
    const objParamAdd = { templateId: this.modelAdd.id };
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(state, "tenant/approvetemplate/getapprovetemplate");
  };

  public dataStateChange(state: DataStateChangeEventArgs): void {
    this.pageIndex = Math.floor(state.skip! / state.take!);
    let extraParams: any[] = [];
    this._coreService.execute(state, "tenant/approvetemplate/getapprovetemplate", extraParams);
  }
  // Số thứ tự
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        this.router.navigate(["/cms/system/approvetemplate/edit/new"]);
        break;
      case ToolbarItem.EDIT:
        ...
        if (selectRows && selectRows.length > 0) {
          ...
          const objParamAdd = { templateId: this.modelAdd.id };
          const paramAdd = window.btoa(JSON.stringify(objParamAdd));
          this.router.navigate(["/cms/system/approvetemplate/edit/", paramAdd]);
        } else {
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (lstDeleteIds.length > 0) {
        this._coreService
          .Post("tenant/approvetemplate/deleteapprovetemplate", lstDeleteIds)
