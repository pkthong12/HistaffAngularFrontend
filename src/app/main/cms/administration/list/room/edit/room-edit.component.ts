...
import { Room } from "src/app/_models/app/cms/index";
...
export class RoomEditComponent implements OnInit {
    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("hr/room/get?id=" + this.paramId)
              .subscribe((res: any) => {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["cms/administration/list/room"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState === "new") {
      this._coreService.Post("hr/room/add", param).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("hr/room/Update", param).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
