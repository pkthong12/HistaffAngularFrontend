...
import { EnumFilterOperator, EnumSortDirection, IFilterOperator, IInOperator, IPagination, ISearchItem, ISortItem } from 'src/app/interfaces/IQueryListRequest';
...
import { ICorePageListApiDefinition, ICorePageListCRUD, ICorePageListEditRouting, ICoreTableFlatColumnItem } from 'src/app/libraries/core-page-list/core-page-list.component';
...
export class TimeImportComponent extends BaseComponent implements AfterViewInit {
        ...
        if (x.ok && x.status == 200) {
          ...
          if (body.statusCode == 200) {
            ...
            newOptions.push(
              {
                value: 1,
                text: this.mls.trans(EnumTranslateKey.UI_CORE_PARAMS_COMMON_WEEKEND_STATUS, this.lang),
                checked: true,
              })
    ...
    this.subscriptions.push(
      this.searchStream$.pipe(
        ...
        if (x.length != 0) {
          ...
          this.onGetList(this.orgIds, this.employeeCodeSearch, this.employeeNameSearch, this.departmentNameSearch, this.positionNameSearch, this.salPeriodObjSearch, this.statuses.join(','));
        }
  ...
  onGetList(listOrgsId: number[], employeeCode: string, employeeName: string, departmentName: string, positionName: string, salPeriodObj: string, codeColorStr: string) {
  ...
  onCorePageHeaderButtonClick(e: ICoreButtonVNS): void {
    switch (e.code) {
      ...
      case EnumCoreButtonVNSCode.HEADER_EDIT:
        this.router.navigate([btoa(this.selectedIds.toString())], { relativeTo: this.route })
        break;
      default:
        break;
    }
    ...
    if (this.salPeriod == null) {
      ...
      if (fileInput.value != '') {
        ...
        blobToBase64(blob).then((base64: any) => {
          ...
          this.http.post(api.AT_TIME_TIMESHEET_DAILY_IMPORT_TEMP, requestBody, { responseType: 'blob' })
            .subscribe(
              (response: Blob) => {
