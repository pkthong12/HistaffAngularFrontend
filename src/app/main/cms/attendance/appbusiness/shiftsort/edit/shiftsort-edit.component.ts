...
export class ShiftSortEditComponent extends BaseEditComponent {
  ...
  onFormCreated(e: FormGroup): void {
    ...
    setTimeout(() => {
      var emp = this.form.get('id')?.value;
      if( emp != 0 && emp != null){
          ...
          [
            ...
            },
            {
              ...
              validators: [
                ...
                {
                  name: 'checkStartDate',
                  validator: ShiftSortEditComponent.checkStartDate,
            ...
            },
            {
              ...
              validators: [
                ...
                {
                  name: 'checkEndDate',
                  validator: ShiftSortEditComponent.checkEndDate,
                  errorMessage: EnumTranslateKey.UI_FORM_CONTROL_ERROR_MIN,
                },
              ]
            },
  ...
  ngAfterViewInit(): void {
    ...
    this.form.get('minDate')?.patchValue(this.minDate);
    this.form.get('maxDate')?.patchValue(this.maxDate);
  }
  ...
  protected static checkStartDate(date: AbstractControl): any | null {
    let valid = true;
    let errorMessage = '';
    const startDate = date.value;
    const minDate = date.parent?.get('minDate')?.value;
    const maxDate = date.parent?.get('maxDate')?.value;
    if (startDate != '' && startDate != null) {
      if (startDate < new Date(minDate) || startDate > new Date(maxDate)) {
        valid = false;
        errorMessage = EnumTranslateKey.UI_ENTITY_ERROR_DATE_NOT_IN_SALARY_PERIOD;
        return CustomValidators.core('startDate', false, errorMessage)(date);
      } else {
        date.parent?.get('startDate')?.setErrors(null);
      }
    }
  }

  protected static checkEndDate(date: AbstractControl): any | null {
    let valid = true;
    let errorMessage = '';
    ...
    if (endDate != '' && endDate != null) {
      if ((endDate > new Date(maxDate) || endDate < new Date(minDate))) {
        valid = false;
        errorMessage = EnumTranslateKey.UI_ENTITY_ERROR_DATE_NOT_IN_SALARY_PERIOD;
        return CustomValidators.core('endDate', false, errorMessage)(date);
      } else {
        date.parent?.get('endDate')?.setErrors(null);
      }
    } else {
      date.parent?.get('endDate')?.setErrors(null);
    }
