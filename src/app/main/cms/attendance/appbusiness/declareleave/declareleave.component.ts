...
@Component({
  selector: "cms-attendent-declareleave",
  templateUrl: "./declareleave.component.html",
  styleUrls: ["./declareleave.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class DeclareleaveComponent implements OnInit {
  // Varriable Language
  languages: any;
  ...
  viewRecord = (event: any) => {
    ...
    this.router.navigate(["/cms/attendance/business/declareleave/", paramAdd]);
  };

  toolItems$ = new BehaviorSubject<any[]>([ToolbarItem.ADD, ToolbarItem.EDIT, ToolbarItem.DELETE, ToolbarItem.PRINT, ToolbarItem.EXPORT_TEMPLATE, ToolbarItem.IMPORT])

  // Build Toolbar
  buildToolbar = () => {
    const toolbarList = [ToolbarItem.ADD, ToolbarItem.EDIT, ToolbarItem.DELETE, ToolbarItem.PRINT, ToolbarItem.EXPORT_TEMPLATE, ToolbarItem.IMPORT];
    this.toolbar = this.globals.buildToolbar("declareleave", toolbarList!);
  };
  // filter checkbox
  ...
  getListData = (): void => {
    ...
    this._coreService.execute(state, "hr/entitlement/GetAll", extraParams);
  };
  GetEmp = (e: any) => {
  ...
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.ADD:
        ...
        this.router.navigate(["/cms/attendance/business/declareleave/new"]);
        break;
      ...
      case ToolbarItem.EXPORT_TEMPLATE:
        ...
        this._coreService
          .Download("hr/entitlement/ExportTemplate", paramExport)
          .subscribe((response: HttpResponse<Blob>) => {
            let filename: string = "ImportPhepNam.xlsx";
            let binaryData = [];
  ...
  confirmDelete = (status: any): void => {
    if (status === "cancel") {
      ...
      if (this.modelDelete.length > 0) {
        ...
        this._coreService
          .Post("hr/entitlement/Delete", ids)
          .subscribe((res: any) => {
    ...
    if (status === "cancel") {
      ...
      if (this.dataImport) {
        if (this.dataImport.length > 0 && this.dataImport.length < 10000) {
          ...
          this._coreService
            .Download("hr/entitlement/ImportTemplate", param[0])
            .subscribe((response: HttpResponse<Blob>) => {
              ...
              if (response.status == 204) {
                ...
                if (response.body!.type == "application/json") {
                  ...
                  binaryData.push(response.body);
                  if (binaryData.length > 0) {
                    let filename: string = "ImportPhepNamError.xlsx";
                    let downloadLink = document.createElement("a");
                    downloadLink.href = window.URL.createObjectURL(
                      new Blob(binaryData as BlobPart[], { type: "blob" }));
                    downloadLink.setAttribute("download", filename);
                    document.body.appendChild(downloadLink);
                    downloadLink.click();
                    this.notification.warning("Import không thành công! bạn vui lòng kiểm tra lại file ImportPhepNamError.xlsx!");
                  }
                }
              }
            });
