...
import { BehaviorSubject, Subject, Subscription, debounceTime, filter, interval, switchMap } from "rxjs";
...
import { EnumFilterOperator, IFilterOperator, IInOperator, IPagination, ISearchItem } from "src/app/interfaces/IQueryListRequest";
...
import { ICoreTableFlatColumnItem } from "src/app/libraries/core-page-list/core-page-list.component";
...
@Component({
  selector: "cms-app-calculatepayroll-backdate",
  templateUrl: "./calculatepayroll-backdate.component.html",
  styleUrls: ["./calculatepayroll-backdate.component.scss"],
})

export class CalculatePayrollBackdateComponent extends BaseComponent implements AfterViewInit {
  ...
  // HEADER
  title = EnumTranslateKey.UI_COMPONENT_TITLE_PA_PAYROLLSHEET_SUM_BACKDATE;
  ...
  labelList = {
    ...
    salPeriodAdd: EnumTranslateKey.UI_LABEL_CUSTOM_PARAM_KIT_SALARY_PERIOD_ADD,
  ...
  // Salary Period Add
  salaryPeridAddOptions$ = new BehaviorSubject<ICoreDropdownOption[]>([]);
  salaryPeriodAddGetByIdObject$ = new BehaviorSubject<any>(null);
  // Salary Object
  ...
  showRequiredSalPeriodAdd: boolean = false;
    ...
    setTimeout(() => {
      ...
      this.subscriptions.push(
        this.layoutService.contentContainerHeight$.subscribe(x => {
          ...
          this.tableHeight = this.compositionHeight - 128 - 45 - this.layoutService.corePaginationHeight;
  ...
  checkRequest(): Observable<any> {
    return this.appService.get(api.PA_PAYROLLSHEET_SUM_BACKDATE_CHECK_REQUEST + this.requestId)
  }
  ...
  onCorePageHeaderButtonClick(e: ICoreButtonVNS): void {
    switch (e.code) {
      case EnumCoreButtonVNSCode.HEADER_LOCK:
        if (this.salPeriod == null || this.listOrgIds.length == 0) {
          ...
          this.subsctiptions.push(
            this.appService.post(api.PA_PAYROLLSHEET_SUM_BACKDATE_CHANGE_STATUS_PAROX_BACKDATE, {
              statusParoxBackdate: 1,
  ...
  onSalPeriodChange(salPeriod: number): void {
    ...
    if(salPeriod != null){
      this.getNextPeirod(salPeriod)
    }
  }

  onSalPeriodAddChange(salPeriodAdd: number): void {
    this.showRequiredSalPeriodAdd = !this.salPeriod ? true : false;
    this.salPeriodAdd = salPeriodAdd;
  }
  ...
  calculate(): void {
    ...
    if (this.checkParamValue(2) == true) {
      this.loading = true;
      this.subsctiptions.push(
        this.appService.post(api.PA_PAYROLLSHEET_SUM_BACKDATE_HANDLE_REQUEST, {
          orgIds: this.listOrgIds,
          year: this.year,
          periodId: this.salPeriod,
          periodAddId: this.salPeriodAdd,
  ...
  getNextPeirod(periodId: number): void{
    this.subsctiptions.push(
      this.appService.get(api.PA_PAYROLLSHEET_SUM_BACKDATE_GET_NEXT_PERIOD + periodId).subscribe((res: any) => {
        if (!!res.ok && res.status === 200) {
          const body: IFormatedResponse = res.body
          if (body.statusCode === 200 && !!body.innerBody) {
            console.log(body.innerBody);
            this.salPeriodAdd = body.innerBody.id;
            this.getListPeriodAdd(body.innerBody.year)
          }
        }
      })
    )!
  ...
  getListPeriodAdd(year: number) {
    this.subsctiptions.push(

      this.appService.post(api.PA_PAYROLLSHEET_SUM_BACKDATE_GET_LIST_IN_YEAR, { year: year }).subscribe(x => {
        if (x.ok && x.status === 200) {
          const body: IFormatedResponse = x.body
          if (body.statusCode === 200 && !!body.innerBody) {
            const options: { value: number | null; text: string }[] = [];
            ...
            this.salaryPeridAddOptions$.next(options);
          } else {
            //this.responseService.resolve(body)
          }
        } else {
  ...
  getListObjSalary() {
    this.subscriptions.push(
      this.appService.get(api.PA_PAYROLLSHEET_SUM_BACKDATE_GET_OBJ_SALARY_SUB_GROUP).subscribe((x) => {
        if (x.ok && x.status === 200) {
          const body: IFormatedResponse = x.body;
          if (body.statusCode === 200) {
  ...
  onGetList(pageSize: number, pageNo: number) {
    ...
    this.loading = true;
    this.subsctiptions.push(
      this.appService.post(api.PA_PAYROLLSHEET_SUM_BACKDATE_GET_LIST, {
        orgIds: this.listOrgIds,
        year: this.year,
        periodId: this.salPeriod,
        periodAddId: this.salPeriodAdd,
  ...
  checkParamValue(checkType: number): boolean {
    ...
    switch (checkType) {
      case 1:
        ...
        break;
      case 2:
        this.showRequiredSalPeriodAdd = !this.salPeriodAdd ? true : false;
        this.showRequiredPayrollFund = !this.payrollFund ? true : false;
        this.showRequiredPayDate = !this.payDate ? true : false;
        if (this.showRequiredOrgId == true) {
          this.alertService.error(this.mls.trans('common.select.org') + '!', alertOptions)
        }
        if (this.showRequiredYear == false && this.showRequiredSalPeriod == false
          && this.showRequiredSalPeriodAdd == false && this.showRequiredObjSalary == false
  ...
  resetCheckParam(): void {
    ...
    this.showRequiredSalPeriodAdd = false;
