...
export class CalculatePayrollComponent extends BaseComponent implements AfterViewInit {
  ...
  headerFirstRowHeight: number = 40;
  ...
  onComparePayrollFund():void {
    this.subsctiptions.push(
      this.appService.post(api.PA_PAYROLLSHEET_SUM_COMPARE_PAYROLL_FUND, {
        orgIds: this.listOrgIds,
        periodId: this.salPeriod,
        payrollFund: this.payrollFund,
      }).subscribe(x => {
        if (x.ok && x.status === 200) {
          const body: IFormatedResponse = x.body
          if (body.statusCode === 200) {
            if(body.innerBody.messagecode === "HIGHER"){
              this.alertService.info(this.mls.trans('PAYROLL_FUND_LESS_THAN_TOTAL_CSUM5'), alertOptions);
            }            
          } else {
            //this.responseService.resolve(body)
          }
