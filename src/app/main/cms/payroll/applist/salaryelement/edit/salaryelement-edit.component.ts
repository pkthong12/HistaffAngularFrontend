...
import { SalaryElement } from "src/app/_models/app/cms";
setCulture("en");

@Component({
  selector: "cms-profile-salaryelement-edit",
  templateUrl: "./salaryelement-edit.component.html",
  styleUrls: ["./salaryelement-edit.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class SalaryElementEditComponent implements OnInit {
  // Varriable Language
  flagState = "";
  ...
  loadData() {
    Promise.all([this.getById(), this.getGroupElement()]).then((res: any[]) => {
  ...
  getById() {
    return new Promise((resolve) => {
      if (this.paramId) {
        this._coreService
          .Get("payroll/element/get?id=" + this.paramId)
          .subscribe((res: any) => {
            resolve(res.data);
          });
      } else {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["cms/payroll/list/salaryelement"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState === "new") {
      this._coreService.Post("payroll/element/add", param).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("payroll/element/Update", param).subscribe(
        (res: any) => {
