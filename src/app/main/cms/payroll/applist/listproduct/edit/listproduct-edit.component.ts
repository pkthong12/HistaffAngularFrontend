...
import { ListProduct } from "src/app/_models/app/cms";
setCulture("en");

@Component({
  selector: "cms-profile-listproduct-edit",
  templateUrl: "./listproduct-edit.component.html",
  styleUrls: ["./listproduct-edit.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class ListproductEditComponent implements OnInit {
  // Varriable Language
  flagState = "";
  ...
  model: ListProduct = new ListProduct();
  modelTemp: ListProduct = new ListProduct();
  languages: any;
  ...
  getById() {
    return new Promise((resolve) => {
      if (this.paramId) {
        this._coreService
          .Get("payroll/listcurlingproduct/get?id=" + this.paramId)
          .subscribe((res: any) => {
  ...
  buildToolbar = () => {
    setTimeout(() => {
      ...
      this.toolbar = this.globals.buildToolbar("listproduct", toolbarList!);
    }, 200);
  };
  // Event Click Toolbar
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["cms/payroll/list/kpigroup"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState === "new") {
      this._coreService.Post("payroll/listcurlingproduct/add", param).subscribe(
        (res: any) => {
          if (res.statusCode == 400) {
            this.notification.checkErrorMessage(res.message);
          } else {
            this.notification.addSuccess();
            this.router.navigate(["/cms/payroll/list/listproduct"]);
          }
        },
        (error: any) => {
      ...
      this._coreService.Post("payroll/listcurlingproduct/Update", param).subscribe(
        (res: any) => {
