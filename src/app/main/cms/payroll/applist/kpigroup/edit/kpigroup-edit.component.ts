...
import { KpiGroup } from "src/app/_models/app/cms";
setCulture("en");

@Component({
  selector: "cms-profile-kpigroup-edit",
  templateUrl: "./kpigroup-edit.component.html",
  styleUrls: ["./kpigroup-edit.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class KpiGroupEditComponent implements OnInit {
  // Varriable Language
  flagState = "";
  ...
  model: KpiGroup = new KpiGroup();
  modelTemp: KpiGroup = new KpiGroup();
  languages: any;
  ...
  getById() {
    return new Promise((resolve) => {
      if (this.paramId) {
        this._coreService
          .Get("KpiGroup/get?id=" + this.paramId)
          .subscribe((res: any) => {
            resolve(res.data);
          });
      } else {
  ...
  saveData = () => {
    ...
    if (this.flagState === "new") {
      this._coreService.Post("KpiGroup/add", param).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("KpiGroup/Update", param).subscribe(
        (res: any) => {
