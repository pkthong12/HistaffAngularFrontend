...
import { ListMachine } from "src/app/_models/app/cms";
setCulture("en");

@Component({
  selector: "cms-profile-listmachine-edit",
  templateUrl: "./listmachine-edit.component.html",
  styleUrls: ["./listmachine-edit.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class ListMachineEditComponent implements OnInit {
  // Varriable Language
  flagState = "";
  ...
  model: ListMachine = new ListMachine();
  modelTemp: ListMachine = new ListMachine();
  languages: any;
  ...
  getById() {
    return new Promise((resolve) => {
      if (this.paramId) {
        this._coreService
          .Get("payroll/curlingmachine/get?id=" + this.paramId)
          .subscribe((res: any) => {
  ...
  buildToolbar = () => {
    setTimeout(() => {
      ...
      this.toolbar = this.globals.buildToolbar("listmachine", toolbarList!);
    }, 200);
  };
  // Event Click Toolbar
  clickToolbar = (itemButton: any): void => {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        this.router.navigate(["/cms/payroll/list/listmachine"]);
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState === "new") {
      this._coreService.Post("payroll/curlingmachine/add", param).subscribe(
        (res: any) => {
      ...
      this._coreService.Post("payroll/curlingmachine/Update", param).subscribe(
        (res: any) => {
