...
import { ElementGroup } from "src/app/_models/app/cms/index";
...
@Component({
  selector: "app-elementgroup-edit",
  templateUrl: "./elementgroup-edit.component.html",
  styleUrls: ["./elementgroup-edit.component.scss"],
  providers: [FilterService, VirtualScrollService],
  encapsulation: ViewEncapsulation.None,
})
export class ElementGroupEditComponent implements OnInit {
  // Varriable Language
  flagState = "";
  ...
  model: ElementGroup = new ElementGroup();
  modelTemp: ElementGroup = new ElementGroup();
  languages: any;
  ...
  constructor(
    ...
    this.editForm = this._formBuilder.group({
      ...
      dateEffect: ["", [Validators.required]],
      note: [""],
    });
    ...
    async.waterfall(
      [
        (cb: any) => {
          if (this.paramId) {
            this._coreService
              .Get("hr/ElementGroup/get?id=" + this.paramId)
              .subscribe((res: any) => {
    ...
    switch (buttonId) {
      case ToolbarItem.BACK:
        ...
        if (this.flagePopup === true) {
          this.router.navigate(["cms/profile/applist/elementgroup"]);
        }
        break;
  ...
  saveData = () => {
    ...
    if (this.flagState === "new") {
      this._coreService.Post("hr/elementgroup/add", param).subscribe(
        (res: any) => {
          if (res.statusCode == 200) {
            ...
            this.notification.warning("Thang Lương đã tồn tại!");
          } else {
      ...
      this._coreService.Post("hr/elementgroup/Update", param).subscribe(
        (res: any) => {
          if (res.statusCode == 200) {
